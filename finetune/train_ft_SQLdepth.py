...
#for ground truth data check "train_test_inputs/kitti_eigen_train_files_with_gt.txt"  directory = "data_depth_annotated"
import matplotlib
...
def train(model, args, epochs=10, experiment_name="DeepLab", lr=0.0001, root=".", device=None,
    ...
    if should_log:
        ...
        wandb.init(project=PROJECT, name=name, config=args, dir=args.root, tags=tags, notes=args.notes,  mode="offline")
        # wandb.watch(model)
    ################################################################################################
