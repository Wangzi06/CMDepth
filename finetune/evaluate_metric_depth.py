...
sys.path.append('/home/MonoViT/spidepth_eval') 
...
def eval(model, test_loader, args, gpus=None, ):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(sequential):
            ...
            print('A', compute_errors(gt[valid_mask], final[valid_mask]))
            ...
            pred = final
            pred_normalized = pred/80 #(pred - pred.min()) / (pred.max() - pred.min())
            pred_uint8 = (pred_normalized * 255).astype(np.uint8)
            pred_colored = cv2.applyColorMap(pred_uint8, cv2.COLORMAP_INFERNO)
            pred_colored_rgb = cv2.cvtColor(pred_colored, cv2.COLOR_BGR2RGB)
            pred_image = Image.fromarray(pred_colored_rgb)
            res = os.path.join('result_pred', batch['image_path'][0].replace('/', '__'))
            pred_image.save(res)
            ...
            pred_colored_rgb = cv2.cvtColor(pred_colored, cv2.COLOR_BGR2RGB)
            pred_image = Image.fromarray(pred_colored_rgb)
            res = os.path.join('result_gt', batch['image_path'][0].replace('/', '__'))
            pred_image.save(res)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() == 3:
        ...
        SQLdepth_opt_filename = '@' + sys.argv[2]
...
# mlx worker launch --gpu 1 --memory 32 --type v100-32g python3 ./finetune/evaluate_metric_depth.py ./finetune/txt_args/eval/eval_nyu.txt ./conf/b5_in_conf.txt 
