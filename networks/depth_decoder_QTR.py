...
#from .mlp_block import MLPEncoder
class Depth_Decoder_QueryTr(nn.Module):
    def __init__(self, in_channels, embedding_dim=128, patch_size=16, num_heads=6, query_nums=100, dim_out=256, norm='linear', min_val=0.001, max_val=10) -> None:
        ...
        self.mamba_blocks = nn.ModuleList([MambaEncoder(d_model=embedding_dim) for _ in range(num_heads)])
        #self.mlp_blocks = nn.ModuleList([MLPEncoder(input_dim=embedding_dim*80) for _ in range(num_heads)])
    ...
    def forward(self, x0):
        ...
        embeddings_0 = embeddings_0.permute(0, 2, 1)  
       
        for mamba_layer in self.mamba_blocks:
            total_queries = mamba_layer(embeddings_0)  
        total_queries = total_queries.permute(1, 0, 2) 
