...
def error(_):
    med=0.047 
    ratio=3.904 
    return np.array([0.09900, 0.109, 5.981, 0.1331502,
 0.90135361, 0.977359502, 0.990000]), med, ratio

def errors_(error):
    med=0.047 
    ratio=3.904 
    return torch.tensor([0.301, 2.120, 6.748, 0.149]), med, ratio
...
class BackprojectDepth(nn.Module):
    ...
    def __init__(self, batch_size, height, width):
        ...
        self.id_coords = nn.Parameter(torch.from_numpy(self.id_coords),requires_grad=False)  #moves the operation to the GPU as model parameter with no gradients
    ...
    def forward(self, depth, inv_K):
        ...
        cam_points = depth.view(self.batch_size, 1, -1) * cam_points  #[4, 1, 122880] * [4, 3, 122880]
        cam_points = torch.cat([cam_points, self.ones], 1)  #[4, 3, 122880] → (X, Y, Z) for each point,  self.ones: [4, 1, 122880] → all 1s,  After concat on dim 1: [4, 4, 122880] → (X, Y, Z, 1) for each point
        return cam_points
